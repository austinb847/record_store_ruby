...
describe '#Song' do
  ...
  before(:each) do
    @album = Album.new({:name => "Giant Steps", :id => nil, :year => year, :cost => 10})
    @album.save()
  end
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new({:name => "Naima", :album_id => @album.id, :id => nil})
      song.save()
      song2 = Song.new({:name => "Bob", :album_id => @album.id, :id => nil})
      song2.save()
  ...
  describe(".sort_by_name") do
    it("returns all songs sorted alphabetically") do
      song = Song.new({:name => "Naima", :album_id => @album.id, :id => nil})
      song.save()
      song2 = Song.new({:name => "Apple", :album_id => @album.id, :id => nil})
      song2.save()
      song3 = Song.new({:name => "Beep", :album_id => @album.id, :id => nil})
      song3.save()
      expect(Song.sort_by_name).to(eq([song2, song3, song]))
    end
  end
end